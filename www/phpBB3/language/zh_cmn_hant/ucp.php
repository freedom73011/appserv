...
$lang = array_merge($lang, array(
	'TERMS_OF_USE_CONTENT'	=> '當您使用「%1$s」（以下以「我們」、「我們的」、「%1$s」、「%2$s」代表）時，即表示您已同意接受本服務條款之所有內容。如果您不同意本服務條款的內容，請您停止存取和/或使用「%1$s」。我們或許會於任何時間修改或變更本服務條款之內容，雖然我們也會盡力通知您任何條款的修改或變更，但仍建議您在使用「%1$s」時隨時注意是否有修改或變更。您於任何修改或變更後繼續使用本服務，將視為您已同意接受該條款的修改或變更。
	<br><br>
	...
	'PRIVACY_POLICY'		=> '這個隱私權保護政策說明「%1$s」以及其相關網站（以下以「我們」、「我們的」、「%1$s」、「%2$s」代表）以及 phpBB（以下以「他們」、「他們的」、「phpBB 軟體」、「www.phpbb.com」、「phpBB Group」、「phpBB Teams」代表）如何處理當會員使用本服務時收集到的個人資料（以下以「您的個人資料」、「個人資料」代表）。
	<br><br>
	我們使用兩種方式來收集您的個人資料。第一，當瀏覽「%1$s」時 phpBB 軟體會產生一些 Cookies，這些小型的文字檔案會儲存在您的電腦的網頁瀏覽器暫存資料夾裡。頭兩個 Cookie 會儲存您的識別編號（以下以「user-id」代表）和一個匿名的 session 識別編號（以下以「session-id」代表），phpBB 軟體將會自動幫您產生這些編號。第三個 Cookie 將會在您瀏覽「%1$s」裡的主題時自動產生並且儲存哪一些主題已被您閱讀，讓您的瀏覽經驗變得更好。
	<br><br>
	當您瀏覽「%1$s」時，除了上述提到的由 phpBB 軟體產生的 Cookies 外，我們或許也會使用其它的外部 Cookies 來儲存資訊。不過這已經超出這個文章的描述範圍，在此，我們僅會說明與 phpBB 軟體相關的部分。第二個收集您的個人資料的方式則是需要由您親自提供給我們。這些可能是（包括但不限於）以匿名用戶的方式發表文章（以下以「匿名文章」代表）、在「%1$s」註冊為會員（以下以「您的帳號」代表）以及當您註冊和登入後所發表的文章（以下以「您的文章」代表）。
	<br><br>
	您的帳號的主要資訊為：一個獨特的識別名稱（以下以「您的會員名稱」代表），一個讓您登入到您的帳號的個人密碼（以下以「您的密碼」代表）以及一個有效的個人電子郵件位址（以下以「您的電子郵件」代表）。在「%1$s」中，您的帳號所包含的個人資料是由這個網站所在國家或地域的資料保護法所保護。除了您的會員名稱、您的密碼以及您的電子郵件以外，我們有權決定哪一些額外資訊是您必須或非必須提供給「%1$s」的。這些非必須的額外資訊，您有權決定哪一些資訊是可以對外公開的。除此之外，在您的帳號中，您可以選擇是否要接收來自 phpBB 軟體內部所寄發的電子信件。
	<br><br>
	您的密碼已經（單向雜湊演算法）加密處理過，因此它是安全的。但是，我們建議您不要在多個網站重複使用相同的密碼。您的密碼是讓您在「%1$s」存取以及使用您的帳號的方法，所以，請您務必要小心保護它。此外，不論在何種情況下「%1$s」、phpBB 或是任何第三方公司的任何人都不會跟您索取您的密碼。如果您忘記了您的帳號的您的密碼，您可以使用 phpBB 軟體提供的「我忘記了自己的密碼」功能。這個程序將會要求您提供您的會員名稱以及您的電子郵件，之後 phpBB 軟體會幫您產生一組新的密碼以讓您繼續使用您的帳號。
	',
));
...
$lang = array_merge($lang, array(
	...
	'ATTACHMENT_LOCKED'				=> '這個主題已鎖定，您無法刪除附加檔案。',
	...
	'BOARD_DATE_FORMAT_EXPLAIN'	=> '格式語法與 PHP <a href="https://secure.php.net/manual/function.date.php">date()</a> 函數相同。',
	...
	'INVALID_EMOJIS_USERNAME'	=> '會員名稱包含禁用字元 (Emoji)。',
	...
	'MESSAGE_REPORTED_MESSAGE'		=> '檢舉的私訊',
	...
	'NOTIFICATION_TYPE_FORUM'							=> '當有人回覆您所訂閱的版面中之主題時',
	...
	'NOTIFICATION_TYPE_REPORT_CLOSED'					=> '您檢舉的文章已被版主結案',
	'NOTIFICATION_TYPE_REPORT_PM'						=> '有人檢舉私訊',
	'NOTIFICATION_TYPE_REPORT_PM_CLOSED'				=> '您檢舉的私訊已被版主結案',
	...
	'NO_AUTH_PRINT_MESSAGE'			=> '您沒有列印私人訊息的權限。',
	...
	'EMAIL_NOT_UNIQUE'			=> '您指定的電子郵件有多個使用者。您還必須指定會員名稱。',
	'NO_FOES'					=> '黑名單是空的',
	'NO_FRIENDS'				=> '好友是空的',
	...
	'PASS_TYPE_ALPHA_EXPLAIN'	=> '密碼的長度必須至少為 %1$s，並且必須包含混合大小寫英文字母以及數字。',
	'PASS_TYPE_ANY_EXPLAIN'		=> '長度必須至少為 %1$s。',
	'PASS_TYPE_CASE_EXPLAIN'	=> '密碼的長度必須至少為 %1$s，並且必須包含混合大小寫英文字母。',
	'PASS_TYPE_SYMBOL_EXPLAIN'	=> '密碼的長度必須至少為 %1$s，並且必須包含混合大小寫英文字母、數字以及特殊符號。',
	'PASSWORD'					=> '密碼',
	'PASSWORD_ACTIVATED'		=> '您的新密碼已經啟用。',
	'PASSWORD_RESET'			=> '您的密碼已成功重設。',
	'PASSWORD_RESET_LINK_SENT'	=> '如果您的帳號存在，則會將密碼重設連結發送到您的註冊電子郵件地址。如果您沒有收到電子郵件，則可能是因為您被禁止、您的帳號未啟動、您在短時間內請求多次密碼重設，或者您不被允許更改密碼。如果有任何上述原因，請與管理員聯繫。另外，請檢查您的垃圾郵件匣。',
	...
	'RECIPIENTS'						=> '收件人',
	'REGISTRATION'						=> '註冊',
	'OAUTH_REGISTRATION'				=> '使用外部伺服器進行註冊',
	...
	'REPORT_PM'							=> '檢舉私訊',
	'RESET_PASSWORD'					=> '重設密碼',
	'RESET_TOKEN_EXPIRED_OR_INVALID'	=> '您提供的密碼重設令牌無效或已過期。',
